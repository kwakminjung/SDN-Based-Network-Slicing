...
```
클라이언트 (S1)          엣지 클라우드 (S_edge)           코어 클라우드 (Score)
...
- **S1–S_edge, S_edge–Score**: 100 Mbps
- **호스트–스위치**: 1 Gbps
- **OpenFlow**: 1.3
...
```
URLLC: S1 → S_edge → [nfv_fw]                    → Score → AutoDrive Hub
eMBB:  S1 → S_edge → [nfv_fw] → [nfv_cache]      → Score → EntertainPort
mMTC:  S1 → S_edge → [nfv_fw] → [nfv_aggr]       → Score → CityPulse Hub
```
...
├── topology.py                  # 3스위치 토폴로지 (S1 + S_edge + Score + NFV 호스트)
├── nfv/
...
│    URLLC: in_port=S1 → nfv_fw → score               │
│    eMBB:  in_port=S1 → nfv_fw → nfv_cache → score   │
│    mMTC:  in_port=S1 → nfv_fw → nfv_aggr  → score   │
│                                                      │
│  Score: dst_ip → 서버 포트 포워딩                    │
└──────────────────────────────────────────────────────┘
        │ OpenFlow FlowMod
        ▼
┌─────────────────────────────────────────────────────┐
...
| 10 | Score | `IP + dst` | `output(server_port)` | 서버 포워딩 |
...
sudo ovs-ofctl dump-flows score   # Score 서버 포워딩 룰
