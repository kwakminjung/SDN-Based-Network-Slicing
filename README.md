...
```
# device_01 연결 시 — prefix 없음 → 기본값 mmtc 배정 후 Gemma3 비동기 판단
Auto-classified: device_01 → MMTC (기본값, Gemma3 pending)

# Gemma3 판단 완료 → urllc로 override, flow rule 재설치
Gemma3: device_01 → urllc
        (device_01은 자율주행 및 V2X와 관련된 트래픽을 다루므로 URLLC가 적합)
...
# 모호한 hostname → mmtc 기본 배정 후 Gemma3 비동기 판단 → override
py add_client(net, 'device_01',  s1)   # → Gemma3 판단 후 슬라이스 결정
```
...
iperf3는 Mininet에서 cookie 핸드셰이크 단계(`unable to receive cookie at server`)에서 실패합니다. iperf3가 TCP 제어 연결과 데이터 교환을 반복하는 과정에서 같은 물리 머신의 네임스페이스 간 통신에 타임아웃이 발생합니다.  
**해결**: **iperf v2** 사용. cookie 핸드셰이크가 없어 Mininet 환경에서 안정적으로 동작합니다.

### scapy 설치 방법

NFV 스크립트는 Mininet 네임스페이스 내 시스템 Python에서 실행되므로 `pip install scapy`로는 적용되지 않습니다.  
...
| `feature/sfc` | **SFC 기반 재설계** — NFV 체인 물리 경유 + 동적 hostname 분류 + 검증 완료 |
